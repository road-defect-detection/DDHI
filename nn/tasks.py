...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, nn.Conv2d, nn.ConvTranspose2d, DWConvTranspose2d, C3x,
                 RepC3,
                 ConvNormLayer, HFIM, DADM):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in (
            BottleneckCSP, C1, C2, C2f, C3, C3x, RepC3):
